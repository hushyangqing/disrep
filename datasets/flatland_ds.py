...
class ForwardVAEDSIID(Dataset):
    def __init__(self, path_input, path_target, path_action, transforms=None, output_targets=True, mean_channels=False, num_steps=1, noise_name=None):
        """ FlatLand Dataset
        ...
        """
        self.inputs = np.load(path_input).reshape(-1, 64, 64, 3).transpose((0, 3, 1, 2))
        self.targets = np.load(path_target).reshape(-1, 64, 64, 3).transpose((0, 3, 1, 2))
        self.actions = np.load(path_action)
        self.actions = np.array(self.actions).reshape(-1, 1)
        self.transforms = transforms
        self.output_targets = output_targets
...
class ForwardHVAEDS(Dataset):
    def __init__(self, path_input, path_action, transforms=None, output_targets=True, mean_channels=False, num_steps=1, noise_name=None, ratio=0.1, mode='train'):
        """ FlatLand Dataset
        ...
        self.ratio = ratio
        self.mode = mode
        length = int(len(self.inputs)*(1-self.ratio))
        self.get_factors()
